...
class SheBoostSupabaseUser extends BaseAuthUser {
  SheBoostSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sheBoostSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SheBoostSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
